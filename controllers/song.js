...
function uploadFile(req, res){
    var songId = req.params.id;
    var file_name= 'No se ha subido el archivo.';
    ...
    if(req.files){
        ...
        res.status(200).send({message: ' No has subido el archivo de la cancion...'})
    }
};
...
function getSongFile(req, res){
    ...
    fs.exists(path_file, function(exists) {
        if(exists){
            ...
            res.status(200).send({message: 'No existe el archivo de la cancion...'})
        }
    });
};
